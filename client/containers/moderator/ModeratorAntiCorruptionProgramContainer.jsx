...
import ModeratorAntiCorruptionProgram from '../../components/moderator/ModeratorAntiCorruptionProgram.jsx';
import ModeratorAntiCorruptionProgramHelmet from '../../helmets/ModeratorAntiCorruptionProgramHelmet.jsx';

import {AntiCorruptionProgramCollection} from '../../../api/AntiCorruptionProgram.js';

import {notReady} from '../../lib/coreLib.js';

class ModeratorAntiCorruptionProgramContainer extends Component {
  constructor(props) {
    super(props);
  ...
  setBlobs() {
    Tracker.autorun((c) => {
      fractionArray = AntiCorruptionProgramCollection.find().fetch();
  ...
  editRegionButtonHandler(e) {
    ...
    const ru = getEditorValue('antiCorruptionProgramru'),
          kz = getEditorValue('antiCorruptionProgramkz'),
          blobs = {
            ru: this.state.blobsru,
            kz: this.state.blobskz
          };

    Meteor.call('antiCorruptionProgram.edit', "1", { ru, kz, blobs }, (err)=> {
      if(err) {
        Bert.alert(err.reason, 'danger', 'growl-top-right' );
      }
      else {
        Bert.alert('Антикоррупционная программа успешно изменена', 'success', 'growl-top-right' );
      }
    });
  }
  ...
  render() {
    const {antiCorruptionProgram} = this.props,
          ...
          currentAntiCorruptionProgram = antiCorruptionProgram.filter((antiCorruptionProgramItem) => antiCorruptionProgramItem._id === "1")[0];

    return (
      <div>
        <ModeratorAntiCorruptionProgramHelmet />
        <ModeratorAntiCorruptionProgram
          editRegionButtonHandler={this.editRegionButtonHandler.bind(this)}
          currentAntiCorruptionProgram={currentAntiCorruptionProgram}
...
export default createContainer(() => {
  return {
    antiCorruptionProgram: AntiCorruptionProgramCollection.find().fetch()
  };
}, ModeratorAntiCorruptionProgramContainer)
