...
import ModeratorElectionProgram from '../../components/moderator/ModeratorElectionProgram.jsx';
import ModeratorElectionProgramHelmet from '../../helmets/ModeratorElectionProgramHelmet.jsx';

import {ElectionProgramCollection} from '../../../api/ElectionProgram.js';

import {notReady} from '../../lib/coreLib.js';

class ModeratorElectionProgramContainer extends Component {
  constructor(props) {
    super(props);
  ...
  editRegionButtonHandler(e) {
    ...
    const ru = getEditorValue('electionProgramru'),
          kz = getEditorValue('electionProgramkz'),
          blobs = {
            ru: this.state.blobsru,
            kz: this.state.blobskz
          };

    Meteor.call('electionProgram.edit', "1", { ru, kz, blobs }, (err)=> {
      if(err) {
        Bert.alert(err.reason, 'danger', 'growl-top-right' );
      }
      else {
        Bert.alert('Предвыборная программа успешно изменена', 'success', 'growl-top-right' );
      }
    });
  }
  ...
  render() {
    const {electionProgram} = this.props,
          ...
          currentElectionProgram = electionProgram.filter((electionProgramItem) => electionProgramItem._id === "1")[0];

    return (
      <div>
        <ModeratorElectionProgramHelmet />
        <ModeratorElectionProgram
          editRegionButtonHandler={this.editRegionButtonHandler.bind(this)}
          currentElectionProgram={currentElectionProgram}
...
export default createContainer(() => {
  return {
    electionProgram: ElectionProgramCollection.find().fetch()
  };
}, ModeratorElectionProgramContainer)
