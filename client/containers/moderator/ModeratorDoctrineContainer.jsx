...
import ModeratorDoctrine from '../../components/moderator/ModeratorDoctrine.jsx';
import ModeratorDoctrineHelmet from '../../helmets/ModeratorDoctrineHelmet.jsx';

import {DoctrineCollection} from '../../../api/Doctrine.js';

import {notReady} from '../../lib/coreLib.js';

class ModeratorDoctrineContainer extends Component {
  constructor(props) {
    super(props);
  ...
  setBlobs() {
    Tracker.autorun((c) => {
      fractionArray = DoctrineCollection.find().fetch();
  ...
  editRegionButtonHandler(e) {
    ...
    const ru = getEditorValue('doctrineru'),
          kz = getEditorValue('doctrinekz'),
          blobs = {
            ru: this.state.blobsru,
            kz: this.state.blobskz
          };

    Meteor.call('doctrine.edit', "1", { ru, kz, blobs }, (err)=> {
      if(err) {
        Bert.alert(err.reason, 'danger', 'growl-top-right' );
      }
      else {
        Bert.alert('Доктрина успешно изменена', 'success', 'growl-top-right' );
      }
    });
  }
  ...
  render() {
    const {doctrine} = this.props,
          ...
          currentDoctrine = doctrine.filter((doctrineItem) => doctrineItem._id === "1")[0];

    return (
      <div>
        <ModeratorDoctrineHelmet />
        <ModeratorDoctrine
...
export default createContainer(() => {
  return {
    doctrine: DoctrineCollection.find().fetch()
  };
}, ModeratorDoctrineContainer)
